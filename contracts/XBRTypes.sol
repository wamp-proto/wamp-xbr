...
library XBRTypes {
     ...
     */
    function splitSignature (bytes memory signature_rsv) private returns (uint8 v, bytes32 r, bytes32 s) {
    ...
    function hash(EIP712Domain memory domain_) private returns (bytes32) {
    ...
    function domainSeparator () private returns (bytes32) {
    ...
    function hash (EIP712MemberRegister memory obj) private returns (bytes32) {
        return keccak256(abi.encode(
    ...
    function hash (EIP712MarketJoin memory obj) private returns (bytes32) {
        return keccak256(abi.encode(
